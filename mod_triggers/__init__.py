...
class mod_triggers(object):
    ...
    def post_trigger(self, params, trigger_id):
        ...
        try:
            sql = u"SELECT reaction FROM command_triggers where id = %s AND last_run <= datetime('now', '%s')" % (trigger_id, self.config['trigger_time'])
            trigger = self.DBcursor.execute(sql).fetchone()
            if (type(trigger) != types.NoneType):
                msg = u'%s' % trigger
                msg = self.format_trigger_output(params, msg)
    ...
    def format_trigger_output(self, params, output):
        ret = ''
    ...
    def update_last_run(self, trigger_id):
        self.DBcursor.execute(u"UPDATE command_triggers SET last_run = datetime('now') where id = %s" % trigger_id)
        self.DBconn.commit()

    def get_all(self):
        return self.DBcursor.execute(u"SELECT id, trigger FROM command_triggers ").fetchall()
            ...
            if ('response:' in params.args):
                ...
                for arg in params.args[1:]:
                    if (response):
                        ...
                        trigger = trigger+' '+arg
                reaction = reaction[1:]
                self.DBcursor.execute(u"INSERT INTO command_triggers (created_by,trigger,reaction, created_at, last_run) VALUES (?, ?, ?, datetime('now'), datetime('now', '-2 days'))",(params.target, trigger, reaction))
    ...
    def info(self,params):
        ...
        if(len(params.args)>=1):
            trigger = self.DBcursor.execute(u"SELECT id, reaction, trigger, last_run from command_triggers WHERE trigger like '%"+params.follow+"%' or reaction like '%"+params.follow+"%' ORDER BY Random() LIMIT 1").fetchall()
            if(len(trigger)>0):
